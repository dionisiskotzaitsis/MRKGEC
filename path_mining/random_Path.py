...
def positivePath3(flag,q):
    ...
    driver = GraphDatabase.driver('bolt://localhost:7687', auth=("user", "pass"), database="db")
    for i in dictionary:
        paths = []
        query2 = """
        ...
        query2 = """
                MATCH p=(u:User)-[:REVIEW]-(d:Business)-[:IN_CATEGORY]-(m:Category)-[:IN_CATEGORY]-(bu:Business)
        ...
        if len(values) > 0:
            for p in values:
                path = ['u' + str(p[0]), 'i' + str(p[1]), 'g' + str(p[2]), 'i' + str(p[3])]
                paths.append(path)
...
def positivePath4(flag,q):
    ...
    for i in dictionary:
        ...
        query2 = """
                ...
                WHERE u.id="{uid}" AND bu.id="{bid}"
                RETURN ID(u),ID(d),ID(m),ID(s),ID(bu) ORDER BY m.cat_count DESC LIMIT 1
                """.format(uid=str(dictionary[i]['user_id']), bid=str(dictionary[i]['business_id']))
        results = driver.session().run(query2)
        values = results.values()
        if len(values) > 0:
        ...
        query2 = """
                ...
                WHERE u.id="{uid}" AND bu.id="{bid}"
                RETURN ID(u),ID(s),ID(d),ID(m),ID(bu) ORDER BY s.review_count LIMIT 1
                """.format(uid=str(dictionary[i]['user_id']), bid=str(dictionary[i]['business_id']))
        results = driver.session().run(query2)
        values = results.values()
        if len(values) > 0:
...
def positivePath5(flag,q):
    ...
    for i in dictionary:
        ...
        query2 = """
                ...
                MATCH (u)-[z:WROTE_IN_MONTH]-(m)-[l:REVIEW_MONTH]-(bu)
                WHERE u.id="{uid}" AND bu.id="{bid}" AND a.review_id=b.review_id=z.review_id=l.review_id
                RETURN ID(u),ID(d),ID(m),ID(s),ID(cat),ID(bu) LIMIT 1
                """.format(uid=str(dictionary[i]['user_id']), bid=str(dictionary[i]['business_id']))
        results = driver.session().run(query2)
        values = results.values()
        if len(values) > 0:
        ...
        query2 = """
                MATCH p=(u:User)-[a:WROTE_IN_DAY]-(d:Day)-[:DAY_OF_MONTH]-(m:Month)-[b:REVIEW_MONTH]-(bu:Business)-[:IN_CITY]-(s:City)-[:IN]-(cat:Area)
                WHERE u.id="{uid}" AND bu.id="{bid}" AND a.review_id=b.review_id
                RETURN ID(u),ID(d),ID(m),ID(s),ID(cat),ID(bu) LIMIT 1
                """.format(uid=str(dictionary[i]['user_id']), bid=str(dictionary[i]['business_id']))
        results = driver.session().run(query2)
        ...
        if len(values) > 0:
            for p in values:
                path = ['u' + str(p[0]), 'd' + str(p[1]), 'm' + str(p[2]), 'c' + str(p[3]), 'a' + str(p[4]),
                        'i' + str(p[5])]
                paths.append(path)

        query2 = """
                MATCH p=(u:User)-[a:WROTE_IN_MONTH]-(d:Month)-[:PART_OF]-(m:Season)-[b:REVIEW_SEASON]-(bu:Business)-[:IN_CITY]-(s:City)-[:IN]-(cat:Area)
                WHERE u.id="{uid}" AND bu.id="{bid}" AND a.review_id=b.review_id
                RETURN ID(u),ID(d),ID(m),ID(s),ID(cat),ID(bu) LIMIT 1
                """.format(uid=str(dictionary[i]['user_id']), bid=str(dictionary[i]['business_id']))
        results = driver.session().run(query2)
        ...
        if len(values) > 0:
            for p in values:
                path = ['u' + str(p[0]), 'm' + str(p[1]), 's' + str(p[2]), 'c' + str(p[3]), 'a' + str(p[4]),
                        'i' + str(p[5])]
                paths.append(path)
        ...
        query2 = """
                ...
                MATCH (u)-[z:WROTE_IN_MONTH]-(m)-[l:REVIEW_MONTH]-(bu)
                WHERE u.id="{uid}" AND bu.id="{bid}" AND a.review_id=b.review_id=z.review_id=l.review_id 
                RETURN ID(u),ID(u2),ID(d),ID(m),ID(s),ID(bu) LIMIT 1
                """.format(uid=str(dictionary[i]['user_id']), bid=str(dictionary[i]['business_id']))
        results = driver.session().run(query2)
        values = results.values()
        if len(values) > 0:
...
if __name__ == '__main__':
    ...
    timeFile=open('paths_Random/timeExtractionRandom.txt','a+')
    ######### CHANGE THE FILEPATH FOR EACH LEN OF PATH ##################
    #####################################################################
    positive_path='paths_Random/positive_path_5.txt'
    negative_path='paths_Random/negative_path_5.txt'
    #####################################################################
    #####################################################################
